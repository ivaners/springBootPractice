package com.code.demo.shiro.filter;


import com.alibaba.fastjson.JSON;
import com.code.demo.common.util.Result;
import com.code.demo.shiro.token.PasswordToken;
import com.code.demo.common.util.CommonUtil;
import com.code.demo.common.util.IpUtil;
import com.code.demo.common.util.RequestResponseUtil;
    ...
    protected boolean onAccessDenied(ServletRequest request, ServletResponse response) throws Exception {
        ...
        if (isPasswordTokenGet(request)) {
            ...
            try {
                ...
                // 动态秘钥response返回给前端
                Result message = Result.ok("issued tokenKey success")
                        .put("tokenKey",tokenKey).put("userKey", userKey.toUpperCase());
                RequestResponseUtil.responseWrite(JSON.toJSONString(message),response);

            }catch (Exception e) {
                LOGGER.warn("签发动态秘钥失败"+e.getMessage(),e);
                Result message = Result.ok("issued tokenKey fail");
                RequestResponseUtil.responseWrite(JSON.toJSONString(message),response);
            }
            return false;
        }
        ...
        if(isPasswordLoginPost(request)){
            ...
            try {
                ...
                // 返回response告诉客户端认证失败
                Result message = Result.error(1002,"login fail");
                RequestResponseUtil.responseWrite(JSON.toJSONString(message),response);
                return false;
            }catch (Exception e) {
        ...
        // response 告知无效请求
        Result message = Result.error(1111,"error request");
        RequestResponseUtil.responseWrite(JSON.toJSONString(message),response);
        return false;
    }
