...
import com.code.demo.shiro.token.JwtToken;
...
public class BJwtFilter extends BPathMatchingFilter {
    ...
    protected boolean isAccessAllowed(ServletRequest servletRequest, ServletResponse servletResponse, Object mappedValue) throws Exception {
        ...
        if ((null != subject && !subject.isAuthenticated()) && isJwtSubmission(servletRequest)) {
            ...
            try {
                ...
                // 其他的判断为JWT错误无效
                Result message = Result.error(1007,"error Jwt");
                RequestResponseUtil.responseWrite(JSON.toJSONString(message),servletResponse);
                return false;
    ...
    protected boolean onAccessDenied(ServletRequest servletRequest, ServletResponse servletResponse) throws Exception {
        ...
        if (null == subject || !subject.isAuthenticated()) {
            ...
            Result message = Result.error(1006,"error jwt");
            RequestResponseUtil.responseWrite(JSON.toJSONString(message),servletResponse);
        }else {
            //  已经认证但未授权的情况
            // 告知客户端JWT没有权限访问此资源
            Result message = Result.error(1008,"no permission");
            RequestResponseUtil.responseWrite(JSON.toJSONString(message),servletResponse);
        }
        // 过滤链终止
        return false;
